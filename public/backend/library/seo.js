$(document).ready(function () {
    ...
    $("input[name=canonical]").css({
        "padding-left": $(".base-url").outerWidth(),
    });

    $("input[name=canonical]").on("keyup", function () {
        const value = removeUtf8($(this).val());
    ...
    const removeUtf8 = (str) => {
        ...
        // str = str.replace(/ + /g, " ");
        // str = str.replace(/^\-+|\-+$/g, "");
        str = str.replace(/\s+/g, "-"); // Thay thế tất cả khoảng trắng bằng dấu '-'
        str = str.replace(/^-+|-+$/g, ""); // Xóa dấu '-' ở đầu và cuối chuỗi
        str = str.trim();
        // Remove punctuations
